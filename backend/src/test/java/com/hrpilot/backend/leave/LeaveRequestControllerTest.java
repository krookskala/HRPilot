...
class LeaveRequestControllerTest {
    ...
    private LeaveRequestResponse buildResponse(LeaveStatus status) {
        return new LeaveRequestResponse(1L, 1L, "John Doe", LeaveType.ANNUAL,
                LocalDate.of(2026, 4, 1), LocalDate.of(2026, 4, 5), status, "Vacation");
    }
    ...
    void getAllLeaveRequests_returns200() throws Exception {
        ...
        when(leaveRequestService.getAllLeaveRequests(any(Pageable.class))).thenReturn(page);
    ...
    void rejectLeaveRequest_asHRManager_returns200() throws Exception {
        when(leaveRequestService.rejectLeaveRequest(1L)).thenReturn(buildResponse(LeaveStatus.REJECTED));
    ...
    void approveLeaveRequest_alreadyApproved_returns422() throws Exception {
        when(leaveRequestService.approveLeaveRequest(1L))
                .thenThrow(new BusinessRuleException("Only PENDING leave requests can be approved"));
